...
def addDropbox():
    ...
    if(token == "" or dropbox == ""):
        resp = jsonify(
            success=False, message="token or dropbox token is empty")
        return make_response(resp, 400)
...
def checkAlbum():
    ...
    if db.Album.query.filter_by(album=albumName).count() == 1:
        if db.Album.query.filter_by(album=albumName, username=u.username).count() >= 1:
            resp = jsonify(
                success=False, message="You are in this album already")
        else:
            resp = jsonify(success=False, message="album already exists")
...
def getAlbumsUsers():
    ...
    if db.Album.query.filter_by(username=s.username, album=albumName).count() != 1:
        resp = jsonify(success=False, message=s.username +
                       " does not have permition to check users in album: " + albumName)
...
def addUserAlbum():
    ...
    if db.Album.query.filter_by(username=s.username, album=albumName).count() != 1:
        resp = jsonify(success=False, message=user +
                       " does not have permition to add users in album: " + albumName)
        return make_response(resp, 403)
    ...
    if db.Album.query.filter_by(username=user, album=albumName).count() >= 1:
        resp = jsonify(success=False, message=user +
                       " already in album: " + albumName)
        return make_response(resp, 400)
